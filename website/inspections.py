...
def lchealthscore(measure_mean_pitch_length, chain_pitch_length):
    ...
    try:
        ...
        if chain_pitch_length < MIN_PITCH_LENGTH:
            flash(f'Chain pitch length must be greater than or equal to minimum length of chain {MIN_PITCH_LENGTH} mm.', 'error')
        if chain_pitch_length > MAX_PITCH_LENGTH:
            flash(f'Chain pitch length must be less than or equal to maximum chain length {MAX_PITCH_LENGTH} mm.', 'error')
